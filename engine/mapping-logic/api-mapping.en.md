...
[![中文](https://img.shields.io/badge/中文-版本-blue)](api-mapping.md)
...
sequenceDiagram
    ...
    participant Theta as Path Selection θ
    participant Omega as Spectrum Setting ω
    participant Atlas as Potentiality Interface Atlas
    participant Backend as Non-existence Tension Structure
    participant S as Reality Snapshot

    You->>Theta: Attention focus
    You->>Omega: Consciousness state
    Theta->>Atlas: Select interface to invoke
    Omega->>Atlas: Set permission level
    You->>Atlas: O: Send request
    Atlas->>Backend: Query potentiality
    Backend-->>Atlas: Return potentiality data
    Atlas-->>S: Map to snapshot
    S-->>You: Render as psychological experience
```
...
|-------------------|-------|
| Core Generation Formula S | [→ formula-S.en.md](formula-S.en.md) |
| Cosmic Integral Model | [→ integral-model.en.md](integral-model.en.md) |
