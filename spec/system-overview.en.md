...
[![中文](https://img.shields.io/badge/中文-版本-blue)](system-overview.md)
...
→ See [engine/snapshot-service/](../engine/snapshot-service/discrete-generation.en.md)
...
→ See [engine/mapping-logic/](../engine/mapping-logic/api-mapping.en.md)
...
sequenceDiagram
    ...
    participant You as Consciousness Interface<br/>(You/Execution Env)
    participant θ as Consciousness Path θ<br/>(Navigator)
    participant ω as Consciousness Spectrum ω<br/>(Permission/Depth)
    participant Atlas as Potentiality Interface<br/>Atlas (API Doc)
    participant ρS as Non-existence Tension<br/>Structure (Backend)
    participant S as Snapshot S<br/>(Response Data)
    
    Note over You,S: ═══════ Phase 1: Request ═══════
    
    You->>θ: 1. Attention Focus<br/>Determine direction to access
    θ->>Atlas: 2. Path Navigation<br/>θ locates in interface atlas
    θ-->>θ: Probability density sampling<br/>(high-prob paths prioritized)
    
    Note over You,S: ═══════ Phase 2: Auth ═══════
    
    You->>ω: 3. State Detection<br/>Current consciousness spectrum level
    ω->>Atlas: 4. Permission Verification<br/>ω determines accessible interface depth
    
    alt ωₗ (Low-freq)
        Atlas-->>ω: Return: Public interface permission<br/>(Sensory layer)
    else ωₘ (Mid-freq)
        Atlas-->>ω: Return: Standard interface permission<br/>(Symbolic layer)
    else ωₕ (High-freq)
        Atlas-->>ω: Return: Privileged interface permission<br/>(Structural layer)
    end
    
    Note over You,S: ═══════ Phase 3: Process ═══════
    
    Atlas->>ρS: 5. Invocation Forward<br/>Pass (θ, ω) params to backend
    ρS-->>ρS: Manifestation tension activation<br/>(Foreshock system incubation)
    ρS-->>ρS: Multi-possibility superposition<br/>(Quantum superposition analogy)
    
    Note over You,S: ═══════ Phase 4: Confirm ═══════
    
    You->>ρS: 6. Observation Action O<br/>Final confirmation/lock command
    ρS-->>ρS: Superposition collapse<br/>(Select one determinate result)
    
    Note over You,S: ═══════ Phase 5: Render ═══════
    
    ρS->>S: 7. Response Generation<br/>Snapshot S instantiated
    S->>You: 8. Data Return<br/>Snapshot passed to execution env
    You-->>You: 9. Decode Render<br/>S → Psychological experience
    
    Note over You,S: ═══════ Phase 6: Loop ═══════
    
    You->>θ: 10. State Update<br/>Current S becomes next frame's starting point
    θ-->>θ: θ path evolution<br/>(probability density may change)
```
...
- [Rendering Engine →](../engine/snapshot-service/discrete-generation.en.md)
